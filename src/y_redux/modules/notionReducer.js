...
//액션 생성 함수
export function loadNotionList(notions){
  return {type: LOAD, notions}
}
export function createNotionList(camp_name, week_num, notion_data){
  return {type: CREATE, camp_name, week_num , notion_data}
}
...
const notionReducer = (state=initialState, action={}) => {
  switch (action.type){
    case LOAD:{
      return {...state, notionList: action.notions}
    }
    case CREATE:{
      const campName = action.camp_name;
      const nowWeek = action.week_num;
      const new_week_notions = [ action.notion_data, ...state[campName][nowWeek]];
      const copyState = {...state};
      copyState[campName][nowWeek] = new_week_notions
      return {...copyState}
    }
    case REMOVE:{
